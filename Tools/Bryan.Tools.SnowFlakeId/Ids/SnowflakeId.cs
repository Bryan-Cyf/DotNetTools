...
namespace Tools.SnowFlakeId
{
    /// <summary>
    /// 雪花算法
    /// Snowflake算法生成的是64位唯一Id（由41位的timestamp+ 10位自定义的机器码+ 13位累加计数器组成）
    /// </summary>
    public class SnowflakeId
    {
        ...
        private const int TIMESTAMP_LEFT_SHIFT = SEQUENCE_BITS + WORKER_ID_BITS;
        ...
        public SnowflakeId(long? workerId, long sequence = 0L)
        {
            if (workerId == null)
            {
                //如果没配置，则采用随机工作Id
        ...
        public virtual long NextId()
        {
            lock (_lock)
            {
                ...
                return ((timestamp - TWEPOCH) << TIMESTAMP_LEFT_SHIFT) | (WorkerId << WORKER_ID_SHIFT) | _sequence;
            }
        }
